...
void CreateCluster(String CLUSTER_PREFIX) {
    withCredentials([string(credentialsId: 'GCP_PROJECT_ID', variable: 'GCP_PROJECT'), file(credentialsId: 'gcloud-key-file', variable: 'CLIENT_SECRET_FILE')]) {
        sh """
            ...
            gcloud container clusters create --zone $GKERegion $CLUSTER_NAME-${CLUSTER_PREFIX} --cluster-version=1.23 --machine-type=n1-standard-4 --preemptible --num-nodes=3 --network=jenkins-vpc --subnetwork=jenkins-${CLUSTER_PREFIX} --no-enable-autoupgrade
...
pipeline {
    environment {
        ...
        CLUSTER_NAME = sh(script: "echo jm-${GIT_SHORT_COMMIT} | tr '[:upper:]' '[:lower:]' | tr '.' '-'", , returnStdout: true).trim()
...
//          Temporally increase build timeout. Should be return to 3 hours in K8SPXC-630
            ...
            parallel {
                ...
                stage('E2E Basic Tests') {
                    steps {
                        CreateCluster('basic')
                        runTest('custom-replset-name', 'basic')
