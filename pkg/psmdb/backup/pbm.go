...
func GetRunningPBMOperation(ctx context.Context, cl *clientcmd.Client, pod *corev1.Pod) (*pbmRunningOp, error) {
	stdoutBuffer := bytes.Buffer{}
	stderrBuffer := bytes.Buffer{}
...
func WaitForPBMOperation(ctx context.Context, cl *clientcmd.Client, pod *corev1.Pod) error {
	log := logf.FromContext(ctx)
	...
	err := retry.OnError(backoff, func(err error) bool { return errors.Is(err, waitErr) }, func() error {
		running, err := GetRunningPBMOperation(ctx, cl, pod)
		if err != nil {
