...
func TestStubBackupEnsureCoordinator(t *testing.T) {
	...
	t.Run("create", func(t *testing.T) {
		...
		// test failures
		client.On("Create", mock.AnythingOfType("*v1.StatefulSet")).Return(nil).Once()
		client.On("Create", mock.AnythingOfType("*v1.Service")).Return(testutils.MockUnexpectedError).Once()
		assert.Error(t, c.EnsureCoordinator())
		client.On("Create", mock.AnythingOfType("*v1.StatefulSet")).Return(testutils.MockUnexpectedError).Once()
		assert.Error(t, c.EnsureCoordinator())
		client.AssertExpectations(t)
	})

	t.Run("update", func(t *testing.T) {
		client.On("Create", mock.AnythingOfType("*v1.StatefulSet")).Return(testutils.MockAlreadyExistsError).Once()
		client.On("Create", mock.AnythingOfType("*v1.Service")).Return(testutils.MockAlreadyExistsError).Once()
...
func TestStubBackupDeleteCoordinator(t *testing.T) {
	...
	// test failures
	client.On("Delete", mock.AnythingOfType("*v1.Service")).Return(nil).Once()
	client.On("Delete", mock.AnythingOfType("*v1.StatefulSet")).Return(testutils.MockUnexpectedError).Once()
	assert.Error(t, c.DeleteCoordinator())
	client.On("Delete", mock.AnythingOfType("*v1.Service")).Return(testutils.MockUnexpectedError).Once()
	assert.Error(t, c.DeleteCoordinator())
}
